...
function parseCssColors (value: string): Rgba[] {
  return Array.from(value.matchAll(/rgba?\(([^)]+)\)/g))
    .map(match => {
      const numbers = match[1].match(/[\d.]+/g)?.map(item => Number(item))
...
function sampleImageLuminance (image: HTMLImageElement, x: number, y: number) {
  ...
  const imageX =
    ((x - rect.left - offsetX) / renderedWidth) * image.naturalWidth
  const imageY =
    ((y - rect.top - offsetY) / renderedHeight) * image.naturalHeight
...
function getElementVisualLuminance (element: Element, x: number, y: number) {
  ...
  const backgroundImageLuminance = averageColorLuminance(
    parseCssColors(styles.backgroundImage)
  )
  ...
  const backgroundColorLuminance = averageColorLuminance(
    parseCssColors(styles.backgroundColor)
  )
      ...
      <div className='section-shell flex h-[var(--header-height)] items-center justify-between gap-4'>
        ...
        >
          ...
          <span className='min-w-0'>
            <span className='block font-serif text-xl text-current'>
              Casa Mimosa
            </span>
            <span className='hidden text-xs text-current/62 sm:block'>
              La Sierrezuela, Fuengirola / Mijas
            </span>
          </span>
        </a>
